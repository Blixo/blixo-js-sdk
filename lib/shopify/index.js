...
const SHOPIFY_CONFIGS = {
  ...
  findSellingPlan: {
    url: "integrations/shopify/findSellingPlan?variant_id=${variant_id}&product_id=${product_id}",
    method: "GET",
    params: {
      page: 1,
      perPage: 10,
...
const Shopify = (configs) => {
  ...
  const findSellingPlan = async ({ variant_id, product_id }) => {
    return request({
      // headers: getHeaders(),
      ...getAuthentication(configs),
      ...getRequestInfo(SHOPIFY_CONFIGS.findSellingPlan, { variant_id, product_id }),
    });
  };
