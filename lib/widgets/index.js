...
const SubscriptionWidget = (configs) => {
  ...
  const render = (options = {}, item) => {
    const { purchaseOptions = {}, translations = DEFAULT_TRANSLATIONS } =
      options;
    const { id } = item;
    ...
    const { intervals = [{ count: intervalCount, interval }] } =
      integrationsShopify;
    ...
    const subscriptionOptions = _.map(
      intervals,
      ({ count, interval }, index) => {
        const intervalKey =
          count === 1 ? INTERVAL_SINGLE_KEY_MAP[interval] : interval;
    ...
    const htmlTemplate = `
      ...
      <div>
        <input type="hidden" id="hidden-item-id" name="properties[item_id]" value="${id}" />
        <input type="hidden" id="hidden-description" name="properties[description]" value="${firstSubscriptionDescription}" />
        <input type="hidden" id="hidden-count" name="properties[shipping_interval_frequency]" value="${_.get(
          firstInterval,
          "count"
        )}" />
        <input type="hidden" id="hidden-interval" name="properties[shipping_interval_unit_type]" value="${_.get(
          firstInterval,
          "interval"
        )}" />
        ${allOptions.join("")}
  ...
  const initialize = async (configs) => {
    ...
    const { response: shopifyResponse, errors: shopifyErrors } =
      await shopify.get();
    const renderOptions = _.get(
      shopifyResponse,
      "data.settings.subscriptionWidget"
    );
  ...
  const addEvent = (wrapper = document) => {
    const inputs = wrapper.getElementsByClassName("blixo-type-input");

    // hidden input to provide cart properties
    const inputItemId = document.getElementById("hidden-item-id");
    ...
    for (let input of inputs) {
      input.addEventListener("click", function () {
        ...
        inputItemId.setAttribute("value", input.dataset.id || null);
        inputCount.setAttribute("value", input.dataset.count || null);
        inputDescription.setAttribute(
          "value",
          input.dataset.description || null
        );
        inputInterval.setAttribute("value", input.dataset.interval || null);
      });
    }
  };
