  ...
  const render = (options = {}, item) => {
    ...
    const htmlTemplate = `
      ...
      <div>
        ...
        <input type="hidden" id="hidden-count" name="properties[shipping_interval_frequency]" value="${_.get(firstInterval, 'count')}" />
        <input type="hidden" id="hidden-interval" name="properties[shipping_interval_unit_type]" value="${_.get(firstInterval, 'interval')}" />
        ${allOptions.join('')}
      </div>
    `
    return htmlTemplate
  ...
  const addEvent = (wrapper = document) => {
    ...
    for (let input of inputs) {
      input.addEventListener('click', function () {
        ...
        inputDescription.setAttribute('value', input.dataset.description || null);
